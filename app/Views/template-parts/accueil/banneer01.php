    <section class="principal-ban">
        <div class="principal01">
            <h1>Entreprise, développez votre potentiel !</h1>
        </div>

        <div class="principal02">
            <div class="interm">
                <img src="<?php echo base_url('image/resized/entreprise.fw.png'); ?>" class="main-image" alt="Développez votre potentiel">
            </div>
        </div>

        <div class="product-card01">
            <img src="<?php echo base_url('image/resized/devweb.fw.png'); ?>" alt="Développement web et applications" class="image-separator">
            <div class="separator"></div>
            <div class="backme02">
                <h2>Développement web et applications</h2>
                <p>Création, refonte et maintenance de sites web. Développement d’application web, application mobile et application desktop.</p>
                <a href="#" class="bouton01">En savoir plus</a>
            </div>
        </div>
        <div class="product-card02">
            <div>
                <div class="spacer"></div>
                <img src="<?php echo base_url('image/resized/men-gba721144d_1280.png'); ?>" alt="Marketing digital" class="md">
                <div class="separator"></div>
                <div class="backme02">
                    <h2>Marketing digital</h2>
                    <p>SEO, SEA, Growth hacking, Community management. Nous développons votre visibilité en ligne.</p>
                    <a href="#" class="bouton01">En savoir plus</a>
                </div>
            </div>
        </div>

        <div class="product-card03">
            <img src="<?php echo base_url('image/resized/pencil.png'); ?>" alt="Création graphique" class="image-separator">
            <!-- <div class="separator"></div> -->
            <div class="backme">
                <h2>Création graphique</h2>
                <p>Création de logo, création de bannières web et bannières publicitaires, création de visuels pour vos brochures, création de plan 3D et architectural</p>
                <a href="#" class="bouton02">En savoir plus</a>
            </div>                        
        </div>

        <div class="product-card04">
            <img src="<?php echo base_url('image/resized/presentoir.fw.png'); ?>" alt="Création de supports" class="image-separator">
            <div class="separator"></div>
            <div class="backme02">
                <h2>Création de supports</h2>
                <p>Confection de plaque publicitaire, confection de présentoirs, PLV, impression 3D</p>
                <a href="#" class="bouton01">En savoir plus</a>
            </div>
        </div>

        <div class="news01">
            <div>
                <h3>Vous avez un besoin ?</h3>
                <form action="" method="post">
                    <textarea name="" id="" cols="50" rows="5" placeholder="Décrivez-le nous en quelques mots !"></textarea>
                    <input type="email" name="email" id="email" placeholder="Votre e-mail">
                    <button type="submit" id="mydemand">Soumettre</button>
                </form>
            </div>
        </div>
       
                
            
    </section>
    <div class="back-image"></div>


    


    